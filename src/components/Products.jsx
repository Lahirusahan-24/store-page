...
import { Search, Plus, X, ArrowUpDown, ChevronDown, Edit, Trash2 } from 'lucide-react';
...
const Products = ({ products, setProducts }) => {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            if (file.size > 1 * 1024 * 1024) { // Limit to 1MB
                alert("Image is too large! Please verify that the image size is less than 1MB.");
                e.target.value = "";
                return;
            }
    ...
    const handleSaveProduct = async () => {
        if (!newItem.name || !newItem.qty) {
            alert('Please fill all required fields (Product Name and Quantity).');
            return;
        }

        // Capture data before resetting form
        const productData = {
            name: newItem.name,
        ...
        try {
            if (wasEditing && idToEdit) {
                await updateDoc(doc(db, "products", idToEdit), productData);
            } else {
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.controlsBar}>
                ...
                <button style={styles.addButton} onClick={() => { resetForm(); setIsPopupOpen(true); }}>
                ...
                {filteredProducts.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map((product) => (
                    <div key={product.id} className="product-card">
                        ...
                        <div style={styles.cardActions}>
                            <button onClick={() => handleEditClick(product)} style={styles.actionButton} title="Edit">
                                <Edit size={16} />
                            </button>
                            <button onClick={() => handleDeleteClick(product.id)} style={{ ...styles.actionButton, color: '#ff4444' }} title="Delete">
                                <Trash2 size={16} />
                            </button>
                        </div>
                    </div>
                ...
                <div style={styles.popupOverlay}>
                    <div style={styles.popup}>
                        <div style={styles.popupHeader}>
                            <h2 style={styles.popupTitle}>{isEditing ? 'Edit Product' : 'Add New Product'}</h2>
                        ...
                        <div style={styles.popupContent}>
                            ...
                            <div style={styles.popupActions}>
                                ...
                                <button onClick={handleSaveProduct} style={styles.submitButton}>
